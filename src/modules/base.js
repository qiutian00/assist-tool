import { cookie, addEvent, removeEvent } from '../utils/utils'
...
export default class Base {
    ...
    addEvent(document, 'click', (event) => {
      ...
      if (target.getAttribute('id') == 'assist-open') {
        if (!this.existIgnore()) {
          ...
          const __href = document
            .getElementById('assist-open')
            .getAttribute('assist-href')
  ...
  formatLongText() {
    const { namespace } = this.config
    const longText =
      document.getElementsByClassName(`${namespace}-long-text`) || []
