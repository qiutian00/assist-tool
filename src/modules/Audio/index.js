...
import audioTabText, { symbolsReg } from '../../config/constans'

import styles from './index.scss'
import tmpl from './index.tmpl.js'
const Audio = {
  init(core) {
    const { namespace, url, audioOptions } = core.config
    // console.log('Audio core:', core)
  ...
  registeDom(namespace) {
    ...
    this.pointeReadTab =
      document.getElementById(`${namespace}-pointeread`) || ''
  },
  toggleAudio() {
  ...
  playAudio(text) {
    const { namespace, AudioApi, isAudio, audio, speed, forceSafariPlayAudio } =
      Audio
    ...
    let urlParamStr = '';
    for (let keyName in this.audioOptions) {
      urlParamStr += `${keyName}=${this.audioOptions[keyName]}&`
    }
    let AudioUrl = `${AudioApi}?${urlParamStr}text=${encodeURI(text)}`
    audio.src = AudioUrl
