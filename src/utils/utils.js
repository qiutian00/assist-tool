...
const parseTagText = (target) => {
  ...
  const __role =
    !!target.getAttribute('role') && target.getAttribute('role').toUpperCase()
  ...
  if (target.children.length === 0) {
    ...
    if (__role === 'INPUT' || target.tagName === 'INPUT') {
      console.log(
        `这是一个${__name}:` + target.alt || target.title || target.value
      )
    ...
    if (__role === 'LABEL' || target.tagName === 'LABEL') {
      ...
      if (!!__linkDom && !!__linkDom.type && __linkDom.type == 'radio') {
        console.log(
          `这是一个单选:` + target.alt || target.title || target.innerText
        )
  ...
  if (
    ...
    (target.alt || target.title || target.innerText)
  ) {
    console.log(`${__name} ${target.alt || target.title || target.innerText}`)
    return `${__name} ${target.alt || target.title || target.innerText}`
  }
